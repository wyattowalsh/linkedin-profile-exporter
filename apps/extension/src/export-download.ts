import {
  exportProfile,
  TEXT_EXPORT_FORMATS
} from "@linkedin-profile-exporter/core/exporters";
import type { ExportFormat, Profile } from "@linkedin-profile-exporter/core/schema";

export async function profileToDownload(profile: Profile, format: ExportFormat, filenameTemplate?: string): Promise<{
  filename: string;
  mimeType: string;
  dataUrl: string;
}> {
  const result = await exportProfile(profile, format, exportOptions(filenameTemplate));
  const bytes = typeof result.contents === "string" ? new TextEncoder().encode(result.contents) : result.contents;
...
export async function profileToClipboardText(profile: Profile, format: ExportFormat, filenameTemplate?: string): Promise<string> {
...
export async function copyProfileExport(profile: Profile, format: ExportFormat, filenameTemplate?: string): Promise<void> {
